...
class Phase2Integration:
    ...
    def format_for_display(self, phase2_summary: Dict[str, Any]) -> str:
        """Format Phase-2 summary for text display with safe string formatting"""
        ...
        try:
            ...
            display_text = f"""**Phase-2 AI Analysis (Mistral)**
...
**Key Findings:**"""
            ...
            if isinstance(concerns, list) and concerns:
                display_text += f"\n\n**Areas of Concern:** {', '.join(str(c) for c in concerns)}"
...
def integrate_phase2_analysis(csv_content: str, ollama_url: str = "http://localhost:11434") -> Dict[str, Any]:
    """Main integration function for Phase-2 analysis with safety guarantees"""
    integration = Phase2Integration(ollama_url)
    
    # Process through Phase-2 with full error handling
